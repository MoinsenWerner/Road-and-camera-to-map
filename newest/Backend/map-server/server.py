...
async def fetch_verified_speed_limit(geom):
    # Dummy: nutze Reverse-Geocoding + Verkehrsdaten-API
    coords = geom.centroid.coords[0]
    lat, lon = coords[1], coords[0]
    try:
        async with httpx.AsyncClient() as client:
            await client.get(f"https://api.openstreetmap.org/api/0.6/reverse?lat={lat}&lon={lon}")
            return 50
    except Exception:
        return None
